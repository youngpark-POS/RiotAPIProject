...
def detail(request, nickname_and_tag):
    ...
    context = {
        ...
        "current_rank_solo": " ".join(
            [rank_info["solo"]["tier"], rank_info["solo"]["rank"]]
        ),
        "current_LP_solo": str(rank_info["solo"]["leaguePoints"])
        + ("" if rank_info["solo"]["tier"] == "Unranked" else "LP"),
        "current_rank_flex": " ".join(
            [rank_info["flex"]["tier"], rank_info["flex"]["rank"]]
        ),
        "current_LP_flex": str(rank_info["flex"]["leaguePoints"])
        + ("" if rank_info["flex"]["tier"] == "Unranked" else "LP"),
        "match_list": [],
        "mastery_list": [],
    }
